...
while response != 'end':
	print "Message 1 is currently:"
	print_linewrapped(display_ctext)
	print "Message 2 is currently:"
	...
	for result_index in xrange(results_len): 
            ...
            if (re.search(charset,results[result_index])):
                    flag = '===common_chars===>>>: '
		    print flag + str(result_index) + ': "' + results[result_index] + '"'
            else:
	...
	try:
		...
		if (response < results_len):
			message_or_key = ''
			while (message_or_key != 'm1' and message_or_key != 'm2'):
				message_or_key = raw_input("Is this crib part of message 1 or message 2? Please enter 'm1' or 'm2': ")
				if(message_or_key == 'm1'):
		...
		if (response == 'end'):
			print "Message 1 is: " + display_ctext
			print "Message 2 is: " + display_key
		elif (response == 'none'):
